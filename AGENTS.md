...
- Use `bun run build` for tsdown, `bun run test` for Vitest, `bun run test:ssr` and `bun run test:bundle` for package checks, `bun run lint:all` for oxlint and TypeScript 7, and `bun run format:check` for oxfmt.
